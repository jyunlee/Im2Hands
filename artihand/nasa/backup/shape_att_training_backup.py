...
sys.path.append('/workspace/hand/halo/halo') 
...
class Trainer(BaseTrainer):
    ...
    def eval_step(self, data):
        ...
        left_joints_gt = mano_data['left'].get('joints').to(device)
        right_joints_gt = mano_data['right'].get('joints').to(device)

        left_joints = mano_data['left'].get('pred_joints').to(device) 
        right_joints = mano_data['right'].get('pred_joints').to(device) 

        '''
        camera_mat = torch.zeros(camera_params['camera'].transpose(1,2).shape).double().cuda() 

        for i in range(img.shape[0]):
            org_camera_mat = camera_params['camera'].transpose(1,2)[i]
            org_camera_mat[2, 2] = (org_camera_mat[0, 0] + org_camera_mat[1, 1]) / 2
            camera_mat[i] = org_camera_mat
        '''

        ''' joints pre-processing code segments '''
        left_joints = torch.bmm(xyz_to_xyz1(left_joints.double()), left_root_rot_mat.double())[:, :, :3] 
        ...
        left_pred_root_z = left_joints[:, 0, 2]
        for i in range(img.shape[0]):
            left_joints[i, :, 2] = left_joints[i, :, 2] - left_pred_root_z[i]

        right_joints = torch.bmm(xyz_to_xyz1(right_joints.double()), right_root_rot_mat.double())[:, :, :3] 
        ...
        right_pred_root_z = right_joints[:, 0, 2]
        for i in range(img.shape[0]):
            right_joints[i, :, 2] = right_joints[i, :, 2] - right_pred_root_z[i]

        left_joints_gt *= 0.4
        left_joints_gt = torch.bmm(xyz_to_xyz1(left_joints_gt.double()), left_root_rot_mat.double())[:, :, :3] 
        left_joints_gt = left_joints_gt + (camera_params['left_root_xyz'].cuda().unsqueeze(1)) 
        left_joints_gt = left_joints_gt * torch.Tensor([-1., 1., 1.]).cuda()

        left_joints_gt = torch.bmm(left_joints_gt, camera_params['R'].transpose(1,2).double().cuda()) + camera_params['T'].double().cuda().unsqueeze(1)
        left_gt_root_z = left_joints_gt[:, 0, 2]
        for i in range(img.shape[0]):
            left_joints_gt[i, :, 2] = left_joints_gt[i, :, 2] - left_gt_root_z[i]
       
        right_joints_gt *= 0.4
        right_joints_gt = torch.bmm(xyz_to_xyz1(right_joints_gt.double()), right_root_rot_mat.double())[:, :, :3] 
        right_joints_gt = right_joints_gt + (camera_params['right_root_xyz'].cuda().unsqueeze(1)) 

        right_joints_gt = torch.bmm(right_joints_gt, camera_params['R'].transpose(1,2).double().cuda()) + camera_params['T'].double().cuda().unsqueeze(1)
        right_gt_root_z = right_joints_gt[:, 0, 2]
        for i in range(img.shape[0]):
            right_joints_gt[i, :, 2] = right_joints_gt[i, :, 2] - right_gt_root_z[i]
        ''' end of joints pre-processing code segments '''
        ...
        if joint_only:
            with torch.no_grad():
                left_joints_pred, right_joints_pred = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_points_iou, right_points_iou), (left_joints, right_joints), (left_inputs, right_inputs), bone_lengths=(left_bone_lengths, right_bone_lengths),
                                   sample=self.eval_sample, **kwargs)
            loss = nn.MSELoss()
            eval_dict['joint_err'] = (loss(left_joints_pred, left_joints_gt).item() + loss(right_joints_pred, right_joints_gt).item()) * 1000
        ...
        o3d.io.write_point_cloud("left_joints_gt.ply", pcd)
          
        pcd.points = o3d.utility.Vector3dVector(left_joints_pred[0].cpu().detach().numpy())
        o3d.io.write_point_cloud("left_joints_pred.ply", pcd)
 
        pcd.points = o3d.utility.Vector3dVector(right_joints_gt[0].cpu().detach().numpy())
        o3d.io.write_point_cloud("right_joints_gt.ply", pcd)
          
        pcd.points = o3d.utility.Vector3dVector(right_joints_pred[0].cpu().detach().numpy())
        o3d.io.write_point_cloud("right_joints_pred.ply", pcd)
        '''
        
        with torch.no_grad():
            left_p_out, right_p_out, left_valid, right_valid, left_joints, right_joints = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_points_iou, right_points_iou), (left_joints, right_joints), (left_inputs, right_inputs), bone_lengths=(left_bone_lengths, right_bone_lengths), root_z = (left_pred_root_z, right_pred_root_z), sample=self.eval_sample, **kwargs)
        ...
        print(left_occ_iou_np)
        print('right')
        print(right_p_out.cpu().numpy())
        print(right_occ_iou_np)
        print()
    ...
    def compute_loss(self, data):
        ...
        zero_indices = (left_occ == 0).nonzero()
        zero_indices = zero_indices[:, 1] # assume batch-size 1 data

        random_idx = torch.randperm(zero_indices.shape[0]) 
        zero_indices = zero_indices[random_idx][:left_p.shape[1] // 2]
        zero_indices, _ = torch.sort(zero_indices, 0)

        selected_indices = []

        left_p = left_p.squeeze()
        left_occ = left_occ.squeeze()

        for i in range(zero_indices.shape[0]):
            idx = zero_indices[i] - i

            if idx < 0:
                print('warning')
                exit()
            
            left_p = torch.cat([left_p[:idx], left_p[idx+1:]])
            left_occ = torch.cat([left_occ[:idx], left_occ[idx+1:]])
           
        left_p = left_p.unsqueeze(0)
        left_occ = left_occ.unsqueeze(0)

        right_p = mano_data['right'].get('points').to(device)  
        right_occ = mano_data['right'].get('occ').to(device)  

        zero_indices = (right_occ == 0).nonzero()
        zero_indices = zero_indices[:, 1] # assume batch-size 1 data

        random_idx = torch.randperm(zero_indices.shape[0]) 
        zero_indices = zero_indices[random_idx][:right_p.shape[1] // 2]
        zero_indices, _ = torch.sort(zero_indices, 0)

        selected_indices = []

        right_p = right_p.squeeze()
        right_occ = right_occ.squeeze()
        ...
        for i in range(zero_indices.shape[0]):
            ...
            right_p = torch.cat([right_p[:idx], right_p[idx+1:]])
            right_occ = torch.cat([right_occ[:idx], right_occ[idx+1:]])
           
        right_p = right_p.unsqueeze(0)
        right_occ = right_occ.unsqueeze(0)
        ...
        left_pred, right_pred, left_valid, right_valid, left_joints_pred, right_joints_pred = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_p, right_p), (left_joints, right_joints), (left_inputs, right_inputs), bone_lengths=(left_bone_lengths, right_bone_lengths), root_z = (left_pred_root_z, right_pred_root_z), **kwargs)
        ...
        if self.use_sdf:
            ...
            left_joints_pred, right_joints_pred = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_p, right_p), (left_joints, right_joints), (left_inputs, right_inputs), bone_lengths=(left_bone_lengths, right_bone_lengths),
                                   sample=self.eval_sample, **kwargs)

            loss = self.mse_loss(left_joints_pred, left_joints_gt) + self.mse_loss(right_joints_pred, right_joints_gt)
            loss *= 100
            loss_dict['joints_err'] = loss.item()
        ...
        joint_loss = self.mse_loss(left_joints_pred, left_joints_gt) + self.mse_loss(right_joints_pred, right_joints_gt)
        joint_loss *= 1000
        loss_dict['joints_err'] = joint_loss.item()
        
        occ_loss = self.mse_loss(left_pred, sub_left_occ) + self.mse_loss(right_pred, sub_right_occ)
        '''
        occ_loss = self.mse_loss(left_pred, left_occ) + self.mse_loss(right_pred, right_occ)
