...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, optimizer, skinning_loss_weight=0, device=None, 
        ...
        self.bce_loss = torch.nn.BCEWithLogitsLoss(pos_weight = torch.Tensor([8/3]).cuda())
    ...
    def eval_step(self, data):
        ...
        with torch.no_grad():
            left_p_out, right_p_out, left_valid, right_valid = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_points_iou, right_points_iou), (left_inputs, right_inputs), (left_anchor_points, right_anchor_points), bone_lengths=(left_bone_lengths, right_bone_lengths),
                               sample=self.eval_sample, **kwargs)


        #left_p_out = nn.Sigmoid()(left_p_out) 
        #right_p_out = nn.Sigmoid()(right_p_out) 
...
# point_in_ids = np.random.choice(point_in.shape[0], 1024)
    ...
    def compute_loss(self, data):
        ...
        left_pred, right_pred, left_valid, right_valid = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_p, right_p), (left_inputs, right_inputs), (left_anchor_points, right_anchor_points), bone_lengths=(left_bone_lengths, right_bone_lengths))
        ...
        print(left_pred[0])
        print(sub_left_occ[0])
        #loss = self.mse_loss(left_pred, sub_left_occ) + self.mse_loss(right_pred, sub_right_occ)
        loss = self.bce_loss(left_pred, sub_left_occ) + self.bce_loss(right_pred, sub_right_occ)
