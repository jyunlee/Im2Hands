...
class Trainer(BaseTrainer):
    ...
    def eval_step(self, data):
        ...
        with torch.no_grad():
            left_p_out, right_p_out, left_valid, right_valid, _, _ = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_points_iou, right_points_iou), (left_inputs, right_inputs), (left_anchor_points, right_anchor_points), bone_lengths=(left_bone_lengths, right_bone_lengths),
...
# point_in_ids = np.random.choice(point_in.shape[0], 1024)
    ...
    def compute_loss(self, data):
        ...
        if self.use_sdf:
            ...
            imgs = (img, mask, dp, camera_params)
        ...
        left_pred, right_pred, left_valid, right_valid, pen_left_pred, pen_right_pred = self.model(imgs, (left_root_rot_mat, right_root_rot_mat), (left_p, right_p), (left_inputs, right_inputs), (left_anchor_points, right_anchor_points), bone_lengths=(left_bone_lengths, right_bone_lengths))
        ...
        penetration_loss = (pen_left_pred * pen_right_pred)
        penetration_loss[penetration_loss < 0.2] = 0
        penetration_loss = penetration_loss.sum()
        print(penetration_loss * 0.001)

        loss = self.mse_loss(left_pred, sub_left_occ) + self.mse_loss(right_pred, sub_right_occ) + penetration_loss * 0.001
