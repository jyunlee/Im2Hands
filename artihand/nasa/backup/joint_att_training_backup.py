...
class Trainer(BaseTrainer):
    ...
    def eval_step(self, data):
        ...
        root_z = left_joints[:, 0, 2]
        for i in range(img.shape[0]):
            left_joints[i, :, 2] = left_joints[i, :, 2] - root_z[i]
        #left_joints -= left_joints[:,0,:].unsqueeze(1)
        #left_joints = torch.bmm(left_joints * 1000, camera_params['camera'].transpose(1,2).double().cuda())
        ...
        root_z = right_joints[:, 0, 2]
        for i in range(img.shape[0]):
            right_joints[i, :, 2] = right_joints[i, :, 2] - root_z[i]

        #right_joints -= right_joints[:,0,:].unsqueeze(1)
        #right_joints = torch.bmm(right_joints * 1000, camera_params['camera'].transpose(1,2).double().cuda()) 
        ...
        root_z = left_joints_gt[:, 0, 2]
        for i in range(img.shape[0]):
            left_joints_gt[i, :, 2] = left_joints_gt[i, :, 2] - root_z[i]
        ...
        root_z = right_joints_gt[:, 0, 2]
        for i in range(img.shape[0]):
            right_joints_gt[i, :, 2] = right_joints_gt[i, :, 2] - root_z[i]
        ...
        loss = nn.MSELoss()
        eval_dict['joint_err'] = (loss(left_joints_pred, left_joints_gt).item() + loss(right_joints_pred, right_joints_gt).item()) * 100
